...
在 [Global Credentials](企业内部 Jenkins/manage/credentials/store/system/domain/_/) 中设置全局的凭证。
...
- `Remote directory`：文件传输到远程服务器上的具体目录，会与 Publish over SSH 插件系统配置中的 `Remote directory` 进行拼接，如我们之前设置的目录是 `/home/jenkins`，此处在写入 `某 PC 项目`，那么最终上传的路径为 `/home/jenkins/某 PC 项目`，与之前不同的是，如果此路径不存在时会自动创建，这样设置后，Jenkins 服务器构建后的产物会通过 ssh 上传到此目录，供下一步使用。
...
- `Exec command`
  ...
  #进入远程服务器的目录
  project_dir=/usr/local/nginx/某 PC 项目/${DEPLOYPATH}
  cd $project_dir

  #移动压缩包
  sudo mv /home/jenkins/某 PC 项目/*.tar.gz  .
...
pipeline {
    ...
    stages {
        stage('拉取代码') {
          steps {
            git branch: "${params.delopyTag}", credentialsId: 'xxx', url: 'https://xxx/fe/某文档项目.git'
          }
        }
        stage('安装依赖') {
          steps {
                        ...
                        transfers: [
                            sshTransfer(
                                ...
                                execCommand: '''#!/bin/bash
                                  #进入远程服务器的目录
                                  project_dir=/usr/local/nginx/某 PC 项目_${DEPLOYPATH}/${DEPLOYPATH}
                                  if [ ${DEPLOYPATH} == "ddm"  ]; then
                                     project_dir=/usr/local/nginx/某 PC 项目/dist
                                  fi
                                  cd $project_dir
