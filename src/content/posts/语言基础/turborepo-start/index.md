...
![multirepo-package](./multirepo-package.png)
...
- 工具代码共享：可以将各项目都会用到的工具函数抽成一个个的方法，在`xxx_libs`统一管理，其他包直接引用即可；
...
![turbo-preview-build-next](./turbo-preview-build-next.png)
...
![turbo-cache-force](./turbo-cache-force.png)
...
![monorepo-demo](./monorepo-demo.png)
...
![turborepo-build](./turborepo-build.png)
...
Turborepo 是一个基于 Vite 的构建工具，可以实现构建的增量化、内容哈希、云缓存、并行执行、任务管道、约定式配置等功能，在我们项目中使用一段时间后，感觉速度嘎嘎快 ✨，如果大家有 Monorepo 的需求，推荐大家使用 Turborepo 。
