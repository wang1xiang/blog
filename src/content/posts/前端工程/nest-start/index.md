...
![nest-all-commands](./nest-all-commands.png)
...
![nest-cli-json](./nest-cli-json.png)
...
![nest-origin-start](./nest-origin-start.png)
...
![nest-watch-start](./nest-watch-start.png)
...
![nest-generate-resource](./nest-generate-resource.png)

接着执行 `npm run start:dev` 来启动项目，一个简单的 crud 项目就启动了，在控制台打印中可以看到有这些接口可以使用：

![nest-crud-interface](./nest-crud-interface.png)

get 请求我们可以直接在浏览器中进行测试：

![nest-curd-get](./nest-curd-get.png)
...
![nest-static-html](./nest-static-html.png)
...
![url-param-location](./url-param-location.png)
...
![url-param-http](./url-param-http.png)
...
![url-query-http](./url-query-http.png)
...
![form-urlencoded-http](./form-urlencoded-http.png)
...
![form-data-http](./form-data-http.png)

服务端成功接收到我们上传的文件，并且保存在配置的 `dest` 目录中：

![form-data-dest](./form-data-dest.png)
