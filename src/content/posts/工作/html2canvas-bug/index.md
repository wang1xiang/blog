...
description: "为了验证自己的猜想，浅浅调试一下 html2canvas 的源码，看下 html2canvas 是怎样一个流程，它是如何将 html内转成 canvas 的。"
tags: ["work"]
category: "工作"
image: api
draft: false
...
![html2canvas-export-png](./html2canvas-export-png.png)

检查一下它的 DOM 结构，发现是下面这样，猜测是就是这个原因导致的。

![html2canvas-mark-bug](./html2canvas-mark-bug.png)
...
![html2canvas-point.png](./html2canvas-point.png)

进入 html2canvas 内部，可以看到内部执行的是 renderElement 方法：

![html2canvas-renderElement](./html2canvas-renderElement.png)
...
![html2canvas-validate-element](./html2canvas-validate-element.png)
...
![html2canvas-merge-options](./html2canvas-merge-options.png)

构建配置项，将传入的 `opts` 与默认配置合并，同时初始化一个 `context` 上下文对象（缓存、日志等）：

![html2canvas-context.png](./html2canvas-context.png)
...
![html2canvas-cache](./html2canvas-cache.png)
...
![html2canvas-generate-iframe](./html2canvas-generate-iframe.png)
...
![html2canvas-toIFrame](./html2canvas-toIFrame.png)
...
![html2canvas-foreignObjectRendering](./html2canvas-foreignObjectRendering.png)
...
3. foreignObject 已经支持除 IE 外的主流浏览器
   ...
   ![can-i-use-foreignObject.png](./can-i-use-foreignObject.png)
...
![html2canvas-use-foreignObjectRendering](./html2canvas-use-foreignObjectRendering.png)

进入 ForeignObjectRenderer 类中

![html2canvas-ForeignObjectRenderer](./html2canvas-ForeignObjectRenderer.png)

通过 ForeignObjectRenderer 实例化一个 renderer 渲染器实例，在 ForeignObjectRenderer 构造方法中初始化 `this.canvas` 对象及其上下文 `this.ctx`

![html2canvas-ForeignObjectRenderer-render](./html2canvas-ForeignObjectRenderer-render.png)

调用 render 生成 canvas，进入到 render 方法：

![html2canvas-ForeignObjectRenderer-render1](./html2canvas-ForeignObjectRenderer-render1.png)

render 方法执行很简单，首先通过 createForeignObjectSVG 将 DOM 内容包装到`<foreignObject>`中生成 svg:

![html2canvas-foreignObject-createForeignObjectSVG](./html2canvas-foreignObject-createForeignObjectSVG.png)

生成的 svg 如下所示：

![html2canvas-foreignObject-svg](./html2canvas-foreignObject-svg.png)
...
![html2canvas-foreignObject-canvas.png](./html2canvas-foreignObject-canvas.png)
...
![html2canvas-foreignObject-flow](./html2canvas-foreignObject-flow.png)
...
![html2canvas-canvas-pasteTree](./html2canvas-canvas-pasteTree.png)
...
![html2canvas-canvas-pasteNodeTree.png](./html2canvas-canvas-pasteNodeTree.png)
...
![html2canvas-canvas-ElementContainer](./html2canvas-canvas-ElementContainer.png)
...
![stacking-context](./stacking-contex.png)
...
![html2canvas-canvas-StackingContext](./html2canvas-canvas-StackingContext.png)
...
![html2canvas-canvas-renderStackContent](./html2canvas-canvas-renderStackContent.png)
...
![html2canvas-canvas-renderNode](./html2canvas-canvas-renderNode.png)
...
![html2canvas-canvas-flow](./html2canvas-canvas-flow.png)
...
![html2canvas-renderNodeContent.png](./html2canvas-renderNodeContent.png)
